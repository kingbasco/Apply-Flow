...
function ScreeningReviewModal({row,onClose}:{row:ScreeningRow;onClose:()=>void}){
 ...
 const loadReview=async()=>{const [{data:r,error:re},{data:ps,error:pe}]=await Promise.all([supabase.from('review_assignments').select('id,submission_id,reviewer_id,status,score,notes,decision,created_at,updated_at').eq('submission_id',row.submissionId).order('created_at',{ascending:false}).limit(1).maybeSingle(),supabase.from('profiles').select('id,full_name,role').eq('organization_id',row.applicationId?'':'').limit(0)]);if(re)setReviewError(re.message);if(pe)setReviewError(pe.message);setReview(r||null);if(r){setReviewerId(r.reviewer_id);setReviewStatus(r.status);setReviewDecision(r.decision||'');setReviewScore(r.score==null?'':String(r.score));setReviewNotes(r.notes||'')}};
