...
export function ReviewsWorkspace({applications,onOpen}:{applications:Application[];onOpen:(a:Application)=>void}){
 ...
 return <section><div className="page-heading compact"><div><p className="eyebrow">Human review</p><h1>Reviews</h1><p className="subtitle">Track reviewer workload, assignments and screening decisions.</p></div><button className="secondary-button" onClick={load}>Refresh</button></div>{error&&<div className="form-error page-error">{error}</div>}<div className="stats-grid"><div className="stat-card"><span>Total reviews</span><strong>{stats.total}</strong></div><div className="stat-card"><span>Unassigned</span><strong>{stats.unassigned}</strong></div><div className="stat-card"><span>Assigned</span><strong>{stats.assigned}</strong></div><div className="stat-card"><span>In progress</span><strong>{stats.inProgress}</strong></div><div className="stat-card"><span>Completed</span><strong>{stats.completed}</strong></div></div>{reviewerCounts.length>0&&<div className="card table-card"><div className="card-header"><div><h2>Reviewer workload</h2><p>Current review assignments by reviewer.</p></div><Users size={20}/></div><div className="table-wrap"><table><thead><tr><th>Reviewer</th><th>Assigned</th></tr></thead><tbody>{reviewerCounts.map(r=><tr key={r.name}><td><strong>{r.name}</strong></td><td>{r.count}</td></tr>)}</tbody></table></div></div>}<div className="card table-card"><div className="toolbar"><div className="search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search applicant or programme…"/></div><select aria-label="Filter reviews by status" value={status} onChange={e=>setStatus(e.target.value)}><option value="all">All statuses</option><option value="unassigned">Unassigned</option><option value="assigned">Assigned</option><option value="in_progress">In progress</option><option value="completed">Completed</option></select></div><div className="table-wrap"><table><thead><tr><th>Applicant</th><th>Programme</th><th>Reviewer</th><th>Status</th><th>Decision</th><th>Score</th><th>Updated</th><th></th></tr></thead><tbody>{loading?<tr><td colSpan={8}><div className="loading-card">Loading review operations…</div></td></tr>:filtered.length===0?<tr><td colSpan={8}><div className="table-empty">No review records match your filters.</div></td></tr>:filtered.map(r=><tr key={r.submissionId}><td><strong>{r.applicantName}</strong><span className="table-sub">{r.email||'No email'}</span></td><td>{r.programmeName}</td><td>{r.reviewerName||'Unassigned'}</td><td><span className={'status '+(r.status==='completed'?'green':r.status==='in_progress'?'amber':r.status==='assigned'?'blue':'neutral')}>{r.status.replace('_',' ')}</span></td><td>{r.decision||'—'}</td><td>{r.score==null?'—':r.score}</td><td>{r.updatedAt?new Date(r.updatedAt).toLocaleDateString():'—'}</td><td><button className="text-button" onClick={()=>{const a=applications.find(x=>x.id===r.applicationId);if(a)onOpen(a)}}>Review</button></td></tr>)}</tbody></table></div></div></section>
}
