...
import { ArrowRight, Users, Settings, FileText, ShieldCheck, ClipboardList, Save, Eye, Search, Plus, History, Lock, LockOpen, SlidersHorizontal, MoreHorizontal, Download, X, CheckCircle2 } from 'lucide-react'
...
export function ReviewsWorkspace({applications,organizationId,onOpen}:{applications:Application[];organizationId:string;onOpen:(a:Application)=>void}){
 ...
 return <section>
  ...
  <div className="stats-grid"><div className="stat-card"><span>Total reviews</span><strong>{stats.total}</strong></div><div className="stat-card"><span>Unassigned</span><strong>{stats.unassigned}</strong></div><div className="stat-card"><span>Assigned</span><strong>{stats.assigned}</strong></div><div className="stat-card"><span>In progress</span><strong>{stats.inProgress}</strong></div><div className="stat-card"><span>Completed</span><strong>{stats.completed}</strong></div></div>
  <div className="card table-card">
   ...
   <div className="review-assignment-toolbar">
    <label className="review-assignee-field"><span>Assign selected to</span><select value={assignmentReviewer} onChange={e=>setAssignmentReviewer(e.target.value)}><option value="">Choose a team member…</option>{reviewers.map(p=><option key={p.id} value={p.id}>{p.full_name||'Unnamed member'} · {p.role}</option>)}</select></label>
    <div className="review-selection-meta">{selectedIds.length} selected</div>
    <button className="primary-button" onClick={assignSelected} disabled={!assignmentReviewer||!selectedIds.length||assigning}>{assigning?'Assigning…':'Assign '+(selectedIds.length||'')+' applicant'+(selectedIds.length===1?'':'s')}</button>
   </div>
   <div className="forms-toolbar"><div className="forms-search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search applicant, programme or email…"/></div><div className="forms-filters"><select aria-label="Filter reviews by status" value={status} onChange={e=>setStatus(e.target.value)}><option value="all">All statuses</option><option value="unassigned">Unassigned</option><option value="assigned">Assigned</option><option value="in_progress">In progress</option><option value="completed">Completed</option></select></div></div>
...
export function TeamWorkspace({organizationId,role:workspaceRole}:{organizationId:string;role?:'owner'|'admin'|'reviewer'}){
 ...
 async function invite(){if(!email.trim())return;setInviting(true);setError('');setNotice('');try{const {data,error}=await supabase.functions.invoke('invite-team-member',{body:{organization_id:organizationId,email:email.trim(),full_name:name.trim()||null,role:memberRole}});if(error)throw error;if(data?.error)throw new Error(data.error);setNotice('Invitation sent to '+email.trim()+'.');setEmail('');setName('');setMemberRole('reviewer');setShowInvite(false);await load()}catch(e:any){setError(e.message||'Could not send the invitation.')}finally{setInviting(false)}}
