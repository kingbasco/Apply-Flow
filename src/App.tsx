...
function App() {
  ...
  function openCreate(mode: 'application'|'form' = 'application') { setCreateMode(mode); setCreateError(''); setNewName(''); setNewDescription(''); setNewDeadline(''); setNewTarget(''); setNewParticipantPrefix('APP'); setCreateOpen(true) }
...
function AnalyticsPanel({applications}:{applications:Application[]}){
 const [loading,setLoading]=useState(true),[refreshing,setRefreshing]=useState(false),[rows,setRows]=useState<any[]>([]),[notice,setNotice]=useState(''),[selectedApplicationId,setSelectedApplicationId]=useState('all'),[lastUpdated,setLastUpdated]=useState<Date|null>(null)
 async function load(options:{silent?:boolean}={}){
  if(options.silent)setRefreshing(true);else setLoading(true);setNotice('')
  try{
   const ids=applications.map(a=>a.id)
   if(!ids.length){setRows([]);setLastUpdated(new Date());return}
   const {data,error}=await supabase.from('submissions').select('id,status,submitted_at,application_id,submission_eligibility(status),submission_selections(status),submission_scores(overall_score)').in('application_id',ids).order('submitted_at',{ascending:false})
   if(error)throw error
   setRows(data||[]);setLastUpdated(new Date())
 ...
 const metrics=useMemo(()=>{const scored=selectedRows.filter(r=>r.submission_scores?.[0]?.overall_score!=null),eligible=selectedRows.filter(r=>r.submission_eligibility?.[0]?.status==='eligible'),shortlisted=selectedRows.filter(r=>r.submission_selections?.[0]?.status==='shortlisted'),selected=selectedRows.filter(r=>r.submission_selections?.[0]?.status==='selected'),avg=scored.length?scored.reduce((s,r)=>s+Number(r.submission_scores[0].overall_score),0)/scored.length:0;return{submitted:selectedRows.length,eligible:eligible.length,shortlisted:shortlisted.length,selected:selected.length,scored:scored.length,avg,eligibilityRate:selectedRows.length?eligible.length/selectedRows.length*100:0,shortlistRate:eligible.length?shortlisted.length/eligible.length*100:0,selectionRate:shortlisted.length?selected.length/shortlisted.length*100:0}},[selectedRows])
 ...
 const programmeRows=useMemo(()=>applications.map(application=>{const programmeRows=rows.filter(r=>r.application_id===application.id);return{application,submitted:programmeRows.length,eligible:programmeRows.filter(r=>r.submission_eligibility?.[0]?.status==='eligible').length,shortlisted:programmeRows.filter(r=>r.submission_selections?.[0]?.status==='shortlisted').length,selected:programmeRows.filter(r=>r.submission_selections?.[0]?.status==='selected').length}}).filter(r=>r.submitted>0),[applications,rows])
 if(loading)return <div className="loading-card card">Loading analytics…</div>
 return <section className="analytics-page">
  <div className="analytics-header"><div><p className="eyebrow">Analytics</p><h1>Programme performance</h1><p className="subtitle">See application volume, funnel progress and scoring activity across your programmes.</p></div><div className="analytics-actions"><label className="analytics-select-wrap"><span>Programme</span><select value={selectedApplicationId} onChange={e=>setSelectedApplicationId(e.target.value)}><option value="all">All programmes</option>{applications.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><button className="secondary-button" onClick={()=>load({silent:true})} disabled={refreshing}>{refreshing?'Refreshing…':'Refresh'} <TrendingUp size={15}/></button></div></div>
  {notice&&<div className="form-error page-error">{notice}</div>}
  {!selectedRows.length?<div className="card analytics-empty"><div className="empty-icon"><BarChart3 size={22}/></div><h2>No analytics data yet</h2><p>Once applications are submitted, this page will show your programme funnel, submission trend and scoring data.</p></div>:
  <>
   <div className="analytics-kpi-grid">
    <div className="card analytics-kpi"><div className="stat-icon"><FileCheck2 size={18}/></div><div><span className="analytics-kpi-label">Submissions</span><strong>{metrics.submitted}</strong><small>Total received</small></div></div>
    <div className="card analytics-kpi"><div className="stat-icon"><CheckCircle2 size={18}/></div><div><span className="analytics-kpi-label">Eligible</span><strong>{metrics.eligible}</strong><small>{metrics.eligibilityRate.toFixed(1)}% of submissions</small></div></div>
    <div className="card analytics-kpi"><div className="stat-icon"><Target size={18}/></div><div><span className="analytics-kpi-label">Shortlisted</span><strong>{metrics.shortlisted}</strong><small>{metrics.shortlistRate.toFixed(1)}% of eligible</small></div></div>
    <div className="card analytics-kpi"><div className="stat-icon"><Users size={18}/></div><div><span className="analytics-kpi-label">Selected</span><strong>{metrics.selected}</strong><small>{metrics.selectionRate.toFixed(1)}% of shortlisted</small></div></div>
    <div className="card analytics-kpi"><div className="stat-icon"><BarChart3 size={18}/></div><div><span className="analytics-kpi-label">Average score</span><strong>{metrics.scored?metrics.avg.toFixed(1):'—'}</strong><small>{metrics.scored} scored submission{metrics.scored===1?'':'s'}</small></div></div>
   </div>
   <div className="analytics-grid analytics-grid-top">
    <div className="card analytics-card"><div className="analytics-card-header"><div><p className="eyebrow">Application funnel</p><h2>From submission to selection</h2><p>Each stage is shown against total submissions.</p></div><Workflow size={19}/></div><div className="analytics-funnel">{[['Submitted',metrics.submitted,100],['Eligible',metrics.eligible,metrics.submitted?metrics.eligible/metrics.submitted*100:0],['Shortlisted',metrics.shortlisted,metrics.submitted?metrics.shortlisted/metrics.submitted*100:0],['Selected',metrics.selected,metrics.submitted?metrics.selected/metrics.submitted*100:0]].map(([label,value,pct])=><div className="analytics-funnel-row" key={String(label)}><div className="analytics-funnel-meta"><span>{label}</span><strong>{Number(value)} <small>{Number(pct).toFixed(1)}%</small></strong></div><div className="analytics-progress"><span style={{width:Math.min(100,Number(pct))+'%'}}/></div></div>)}</div></div>
   ...
   <div className="analytics-grid analytics-grid-bottom">
    <div className="card analytics-card"><div className="analytics-card-header"><div><p className="eyebrow">Score distribution</p><h2>How scored applications are spread</h2><p>{metrics.scored} scored submission{metrics.scored===1?'':'s'} included.</p></div><BarChart3 size={19}/></div><div className="analytics-score-list">{scoreBands.map(b=>{const pct=metrics.scored?b.count/metrics.scored*100:0;return <div className="analytics-score-row" key={b.label}><div><span>{b.label}</span><strong>{b.count}</strong></div><div className="analytics-progress"><span style={{width:pct+'%'}}/></div></div>})}</div></div>
    <div className="card analytics-card analytics-rates"><div className="analytics-card-header"><div><p className="eyebrow">Conversion</p><h2>Stage rates</h2><p>Useful context for programme performance.</p></div><Target size={19}/></div><div className="analytics-rate-list"><div><span>Eligibility rate</span><strong>{metrics.eligibilityRate.toFixed(1)}%</strong><small>{metrics.eligible} of {metrics.submitted}</small></div><div><span>Shortlist rate</span><strong>{metrics.shortlistRate.toFixed(1)}%</strong><small>{metrics.shortlisted} of {metrics.eligible} eligible</small></div><div><span>Selection rate</span><strong>{metrics.selectionRate.toFixed(1)}%</strong><small>{metrics.selected} of {metrics.shortlisted} shortlisted</small></div></div></div>
   </div>
   {selectedApplicationId==='all'&&<div className="card analytics-card analytics-programmes"><div className="analytics-card-header"><div><p className="eyebrow">Programme breakdown</p><h2>Performance by programme</h2><p>Compare application volume and outcomes without leaving Analytics.</p></div></div><div className="table-wrap"><table><thead><tr><th>Programme</th><th>Submissions</th><th>Eligible</th><th>Shortlisted</th><th>Selected</th></tr></thead><tbody>{programmeRows.length?programmeRows.map(({application,submitted,eligible,shortlisted,selected})=><tr key={application.id}><td><strong>{application.name}</strong><span className="table-sub">{application.status}</span></td><td>{submitted}</td><td>{eligible}</td><td>{shortlisted}</td><td>{selected}</td></tr>):<tr><td colSpan={5}><div className="table-empty">No submitted applications yet.</div></td></tr>}</tbody></table></div></div>}
   <div className="analytics-footer"><span>{lastUpdated?'Updated '+lastUpdated.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):'Live programme data'}</span><span>Analytics updates from application, screening and selection activity.</span></div>
  </>}
 </section>
}
