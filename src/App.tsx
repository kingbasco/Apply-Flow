...
function ScreeningPanel({applicationId}:{applicationId:string}){
 const [rows,setRows]=useState<any[]>([]),[selected,setSelected]=useState<any|null>(null),[loading,setLoading]=useState(true),[notice,setNotice]=useState('')
 async function load(){setLoading(true);const {data,error}=await supabase.from('submissions').select('id,status,submitted_at,applicants!inner(full_name,email),ai_screenings(id,status,overall_assessment,criterion_assessments,strengths,concerns,missing_information,inconsistencies,evidence,suggested_score,confidence,model,updated_at)').eq('application_id',applicationId).order('submitted_at',{ascending:false});if(error)setNotice(error.message);else setRows(data||[]);setLoading(false)}
 useEffect(()=>{load()},[applicationId])
 async function createScreening(id:string){const {error}=await supabase.from('ai_screenings').insert({submission_id:id,status:'pending'});if(error&&error.code!=='23505')setNotice(error.message);else load()}
 ...
 return <div className="screening-panel">
  ...
  {selected&&<div className="screening-drawer-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setSelected(null)}}><aside className="screening-drawer card"><div className="preview-header"><div><p className="eyebrow">Screening review</p><h2>{selected.applicants?.full_name||'Applicant'}</h2><p>{selected.applicants?.email}</p></div><button className="icon-button" onClick={()=>setSelected(null)}><X size={18}/></button></div>{!selected.ai_screenings?.[0]?<div className="builder-empty"><Sparkles size={22}/><h3>AI screening not started</h3><p>Start an assessment when the screening criteria are configured.</p><button className="primary-button" onClick={()=>createScreening(selected.id)}>Create screening record</button></div>:<div className="screening-detail"><div className="screening-status-card"><span>Status</span><strong>{selected.ai_screenings[0].status}</strong><small>{selected.ai_screenings[0].model||'AI model not assigned'}</small></div><section><p className="eyebrow">Assessment</p><p>{selected.ai_screenings[0].overall_assessment||'No assessment available yet.'}</p></section><section><p className="eyebrow">Suggested score</p><strong className="screening-score">{selected.ai_screenings[0].suggested_score??'—'}</strong></section><div className="screening-columns"><section><p className="eyebrow">Strengths</p>{(selected.ai_screenings[0].strengths||[]).map((x:string,i:number)=><p key={i}>• {x}</p>)}</section><section><p className="eyebrow">Concerns</p>{(selected.ai_screenings[0].concerns||[]).map((x:string,i:number)=><p key={i}>• {x}</p>)}</section></div><section><p className="eyebrow">Missing information</p>{(selected.ai_screenings[0].missing_information||[]).length?(selected.ai_screenings[0].missing_information||[]).map((x:string,i:number)=><p key={i}>• {x}</p>):<p className="muted">None recorded.</p>}</section><div className="screening-note">AI screening is advisory only. A reviewer must make the final decision.</div></div>}</aside></div>}
 </div>
}
function AnalyticsPanel({applicationId}:{applicationId:string}) {
  const [loading,setLoading]=useState(true), [rows,setRows]=useState<any[]>([]), [notice,setNotice]=useState('')
  useEffect(()=>{(async()=>{setLoading(true);const {data,error}=await supabase.from('submissions').select('id,status,submitted_at,submission_eligibility(status),submission_selections(status),submission_scores(overall_score)').eq('application_id',applicationId);if(error)setNotice(error.message);else setRows(data||[]);setLoading(false)})()},[applicationId])
