...
function AnalyticsPanel({applications}:{applications:Application[]}){
 ...
 const metrics=useMemo(()=>{
  ...
  const eligible=selectedRows.filter(r=>r.submission_eligibility?.[0]?.status==='eligible')
  const rejected=selectedRows.filter(r=>r.decision==='rejected')
  const enrolled=selectedParticipants.filter(p=>p.status==='active')
  const completed=selectedParticipants.filter(p=>p.status==='completed')
  const withdrawn=selectedParticipants.filter(p=>p.status==='withdrawn')
  ...
  return{submitted:selectedRows.length,eligible:eligible.length,rejected:rejected.length,enrolled:enrolled.length,completed:completed.length,withdrawn:withdrawn.length,totalParticipants,scored:scored.length,avg,approvalRate:selectedRows.length?totalParticipants/selectedRows.length*100:0,completionRate:totalParticipants?completed.length/totalParticipants*100:0,withdrawalRate:totalParticipants?withdrawn.length/totalParticipants*100:0}
 ...
 const programmeRows=useMemo(()=>applications.map(application=>{const ps=rows.filter(r=>r.application_id===application.id),pp=participants.filter(p=>p.application_id===application.id);return{application,submitted:ps.length,eligible:ps.filter(r=>r.submission_eligibility?.[0]?.status==='eligible').length,enrolled:pp.filter(p=>p.status==='active').length,completed:pp.filter(p=>p.status==='completed').length,withdrawn:pp.filter(p=>p.status==='withdrawn').length}}).filter(r=>r.submitted>0||r.enrolled>0),[applications,rows,participants])
 ...
 return <section className="analytics-page">
  ...
  {!selectedRows.length&&!selectedParticipants.length?<div className="card analytics-empty"><div className="empty-icon"><BarChart3 size={22}/></div><h2>No analytics data yet</h2><p>Once applications are submitted and approved, this page will show your application, participant and programme completion data.</p></div>:
  <>
   <div className="analytics-kpi-grid">
    ...
    <div className="card analytics-kpi"><div className="stat-icon"><CheckCircle2 size={18}/></div><div><span className="analytics-kpi-label">Eligible</span><strong>{metrics.eligible}</strong><small>{metrics.submitted?((metrics.eligible/metrics.submitted)*100).toFixed(1):'0.0'}% of submissions</small></div></div>
    <div className="card analytics-kpi"><div className="stat-icon"><BadgeCheck size={18}/></div><div><span className="analytics-kpi-label">Enrolled</span><strong>{metrics.enrolled}</strong><small>Approved participants</small></div></div>
   ...
   <div className="analytics-grid analytics-grid-top">
    <div className="card analytics-card"><div className="analytics-card-header"><div><p className="eyebrow">Application journey</p><h2>Submission to participation</h2><p>Approved applicants become participants automatically.</p></div><Workflow size={19}/></div><div className="analytics-funnel">{[['Submitted',metrics.submitted,100],['Eligible',metrics.eligible,metrics.submitted?metrics.eligible/metrics.submitted*100:0],['Enrolled',metrics.enrolled,metrics.submitted?metrics.enrolled/metrics.submitted*100:0],['Completed',metrics.completed,metrics.submitted?metrics.completed/metrics.submitted*100:0]].map(([label,value,pct])=><div className="analytics-funnel-row" key={String(label)}><div className="analytics-funnel-meta"><span>{label}</span><strong>{Number(value)} <small>{Number(pct).toFixed(1)}%</small></strong></div><div className="analytics-progress"><span style={{width:Math.min(100,Number(pct))+'%'}}/></div></div>)}</div></div>
   ...
   <div className="card analytics-card analytics-programmes"><div className="analytics-card-header"><div><p className="eyebrow">Programme breakdown</p><h2>Applications and participants</h2><p>See how each programme is progressing without using shortlist stages.</p></div></div><div className="table-wrap"><table><thead><tr><th>Programme</th><th>Submissions</th><th>Eligible</th><th>Enrolled</th><th>Completed</th><th>Withdrawn</th></tr></thead><tbody>{programmeRows.length?programmeRows.map(({application,submitted,eligible,enrolled,completed,withdrawn})=><tr key={application.id}><td><strong>{application.name}</strong><span className="table-sub">{application.status}</span></td><td>{submitted}</td><td>{eligible}</td><td>{enrolled}</td><td>{completed}</td><td>{withdrawn}</td></tr>):<tr><td colSpan={6}><div className="table-empty">No programme activity yet.</div></td></tr>}</tbody></table></div></div>
